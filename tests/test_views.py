...
class TestDashboard:
    ...
    def test_dashboard_loads(self, auth_client):
        ...
        url = reverse('data_export:dashboard')
...
@pytest.mark.django_db
class TestDataJobViews:
    """DataJob view tests."""

    def test_list_loads(self, auth_client):
        """Test list view loads."""
        url = reverse('data_export:data_jobs_list')
        response = auth_client.get(url)
        assert response.status_code == 200
    ...
    def test_add_form_loads(self, auth_client):
        """Test add form loads."""
        url = reverse('data_export:data_job_add')
        response = auth_client.get(url)
        assert response.status_code == 200
    ...
    def test_add_post(self, auth_client):
        ...
        data = {
            ...
            'file_format': 'New File Format',
            'status': 'New Status',
        }
        response = auth_client.post(url, data)
        assert response.status_code == 200

    def test_edit_form_loads(self, auth_client, data_job):
        """Test edit form loads."""
        url = reverse('data_export:data_job_edit', args=[data_job.pk])
        response = auth_client.get(url)
        assert response.status_code == 200

    def test_edit_post(self, auth_client, data_job):
        ...
        data = {
            ...
            'file_format': 'Updated File Format',
            'status': 'Updated Status',
        }
        response = auth_client.post(url, data)
        assert response.status_code == 200

    def test_delete(self, auth_client, data_job):
        """Test soft delete via POST."""
        url = reverse('data_export:data_job_delete', args=[data_job.pk])
        response = auth_client.post(url)
        assert response.status_code == 200
        data_job.refresh_from_db()
        assert data_job.is_deleted is True

    def test_bulk_delete(self, auth_client, data_job):
        """Test bulk delete."""
        url = reverse('data_export:data_jobs_bulk_action')
        response = auth_client.post(url, {'ids': str(data_job.pk), 'action': 'delete'})
        assert response.status_code == 200
        data_job.refresh_from_db()
        assert data_job.is_deleted is True
...
class TestSettings:
    ...
    def test_settings_loads(self, auth_client):
        ...
        url = reverse('data_export:settings')
        response = auth_client.get(url)
        assert response.status_code == 200
